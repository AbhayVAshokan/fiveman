...
describe Fiveman::Export::Systemd, :fakefs, :aggregate_failures do
  let(:procfile)  { write_procfile("/tmp/app/Procfile") }
  let(:formation) { nil }
  let(:engine)    { Fiveman::Engine.new(:formation => formation).load_procfile(procfile) }
  let(:options)   { Hash.new }
  let(:systemd)   { Fiveman::Export::Systemd.new("/tmp/init", engine, options) }
  ...
  context "with alternate template directory specified" do
    ...
    context "with alternate templates in the user home directory" do
      before do
        FileUtils.mkdir_p File.expand_path("~/.fiveman/templates/systemd")
        File.open(File.expand_path("~/.fiveman/templates/systemd/master.target.erb"), "w") do |file|
